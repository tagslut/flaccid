...
def store_credentials(service_name: str, username: str, password: Optional[str] = None):
    ...
    try:
        ...
        if service_info["password_field"] and password:
            keyring.set_password(
                keyring_service, service_info["password_field"], password
            )
    ...
    try:
        ...
        if service_info["password_field"]:
            password = keyring.get_password(
                keyring_service, service_info["password_field"]
            )

        return {"username": username, "password": password, "service": service_name}
    except Exception as e:
...
def apple_auth(
    developer_token: str = typer.Option(
        None, "--developer-token", "-d", help="Apple Music Developer Token"
    ),
    user_token: str = typer.Option(
        None, "--user-token", "-u", help="Apple Music User Token (optional)"
    ),
):
    """
    ...
    if not user_token:
        user_token = Prompt.ask(
            "Apple Music User Token (optional, press Enter to skip)", default=""
        )
    ...
    if store_credentials("apple", developer_token, user_token if user_token else None):
        ...
        if user_token:
            ...
            console.print(
                "   Only Developer token stored (User token can be added later).",
                style="dim",
            )
    else:
...
def spotify_auth(
    ...
    client_secret: str = typer.Option(
        None, "--client-secret", "-s", help="Spotify Client Secret"
    ),
):
    """
...
def list_credentials():
    ...
    for service_name, service_info in SERVICES.items():
        ...
        if credentials and credentials["username"]:
            ...
            status = "🔐 Configured"
            table.add_row(
                service_info["name"], masked_username, password_status, status
            )
        else:
            table.add_row(
                service_info["name"], "Not set", "Not set", "❌ Not configured"
            )
...
def test_credentials(
    service: str = typer.Argument(
        ..., help="Service to test (qobuz, apple, tidal, spotify)"
    )
):
    """
    ...
    if service == "qobuz":
        if credentials["username"] and credentials["password"]:
            ...
            console.print(
                "⚠️  To fully test, try using 'fla tag qobuz' command", style="yellow"
            )
        else:
        ...
        if credentials["username"]:  # Token stored as username
            ...
            console.print(
                "⚠️  To fully test, try using 'fla tag apple' command", style="yellow"
            )
        else:
        ...
        if credentials["username"] and credentials["password"]:
            ...
            console.print(
                "⚠️  To fully test, try using 'fla get tidal' command", style="yellow"
            )
        else:
        ...
        if credentials["username"] and credentials["password"]:  # Client ID/Secret
            ...
            console.print(
                "⚠️  To fully test, try using 'fla tag spotify' command", style="yellow"
            )
        else:
...
def remove_credentials(
    service: str = typer.Argument(..., help="Service to remove credentials for")
):
    """
    ...
    if delete_credentials(service):
        ...
        console.print(
            f"❌ Failed to remove {service_info['name']} credentials.", style="red"
        )
        raise typer.Exit(1)
...
def clear_all():
    ...
    console.print(
        "[bold red]⚠️  This will remove ALL stored service credentials![/bold red]"
    )
    ...
    if removed_count > 0:
        console.print(
            f"✅ Removed credentials for {removed_count} services.", style="green"
        )

    if error_count > 0:
        console.print(
            f"❌ Failed to remove credentials for {error_count} services.", style="red"
        )
...
@app.command("export")
def export_config(
    output: str = typer.Option("flaccid-config.json", help="Output file path")
):
    """
    ...
    config_data = {"exported_at": datetime.now().isoformat(), "services": {}}

    for service_name, service_info in SERVICES.items():
        credentials = get_credentials(service_name)

        config_data["services"][service_name] = {  # type: ignore[index]
    ...
    console.print(
        "ℹ️  Note: Only configuration status exported, not actual credentials.",
        style="blue",
    )
