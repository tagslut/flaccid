...
from flaccid.plugins import DiscogsPlugin, LyricsPlugin
...
def tag_from_id(file: Path, track_id: str) -> None:
    """Fetch metadata for ``track_id`` from Discogs and apply it to ``file``."""

    async def _tag() -> None:
        async with DiscogsPlugin() as api, LyricsPlugin() as lyr:
