...
async def fetch_metadata(track_id: str, provider: str = "apple") -> Dict[str, Any]:
    """Fetch metadata for track from the specified provider."""
    # This would be implemented to use the appropriate plugin
    return {}


async def fetch_and_tag(file_path: Path, metadata: TrackMetadata) -> bool:
    ...
    """
    # This would be implemented to apply metadata to the file
    return True


async def save_metadata(metadata: Dict[str, Any], output_file: Path) -> None:
    """Save metadata to a file for later use.

    Args:
        metadata: Track metadata to save
        output_file: Path to save the metadata to
    """
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=2)


async def load_metadata(metadata_file: Path) -> Dict[str, Any]:
