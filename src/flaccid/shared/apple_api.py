...
class AppleAPI:
    ...
    async def _get_tokens(self):
        ...
        if not self.developer_token:
            ...
            if not self.developer_token:
                ...
                self.developer_token = self.default_token
    ...
    async def get_album(self, album_id: str):
        ...
        if not self.developer_token or self.developer_token == "your_developer_token":
            console.print("⚠️  Apple Music developer token required for album lookup", style="yellow")
            return None

        session = await self._get_session()
        url = f"{self.BASE_URL}/{self.store}/albums/{album_id}"
        ...
        async with session.get(url, headers=headers) as resp:
            if resp.status == 200:
                ...
                console.print(f"❌ Apple Music album lookup failed: {resp.status}", style="red")
                return None
    ...
    async def search_albums(self, query: str, limit: int = 5):
        ...
        if not self.developer_token or self.developer_token == "your_developer_token":
            return await self._itunes_search_albums(query, limit)
        ...
        async with session.get(url, headers=headers, params=params) as resp:
            if resp.status == 200:
                ...
                console.print(f"❌ Apple Music album search failed: {resp.status}", style="red")
                return await self._itunes_search_albums(query, limit)

    async def _itunes_search_albums(self, query: str, limit: int = 5):
        """Search albums using iTunes Search API."""
        session = await self._get_session()
        url = f"{self.ITUNES_BASE_URL}/search"
        ...
        async with session.get(url, params=params) as resp:
            if resp.status == 200:
                ...
                console.print(f"❌ iTunes album search failed: {resp.status}", style="red")
                return None
